...
internal class Rules
{
    ...
    private static int colorCount = 2;
    ...
    internal static void Initialize(int sideLength, int colorCount)
    {
        cacheShiftCount = (int)Math.Ceiling(Math.Log2(ColorCount + 1));
        Rules.colorCount = colorCount;

        maxColorPerLine = sideLength / colorCount;
        cache = new int[DIMENSIONS][,];
        for (int i = 0; i < DIMENSIONS; i++)
            cache[i] = new int[sideLength, sideLength];
    }

    internal static int[][,] GetCache()
    {
        return cache;
    }
